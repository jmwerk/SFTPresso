...
- **Server-side rename and move** — a single remote `rename()` call regardless of size, from the Remote Explorer context menu or automatically via `watcher.autoRename`, instead of a full re-upload
